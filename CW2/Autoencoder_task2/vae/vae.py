...
def mae(y_true, y_pred):
  return K.mean(K.square(K.abs(y_true-y_pred)), axis = -1)
...
i = Dense(intermediate_dim, activation = 'relu')(h)
## We recover here \mu and \sigma
## For stability purposes, we assume that it outputs \log(\sigma)
z_mu = Dense(latent_dim)(i)
z_log_var = Dense(latent_dim)(i)
...
image_inception_score(decoder, dim_random=latent_dim, denorm_im=1)
